    ...
    cassandraPort := "9160",
	  ...
	  cassandraTgz := "",
		...
		deployCassandra := {
			...
			val cassandraTarGz:File = if(cassandraTgz.value.nonEmpty) file(cassandraTgz.value) else Attributed.data(classpath).find(_.getName == s"apache-cassandra-$ver-bin.tar.gz").get
